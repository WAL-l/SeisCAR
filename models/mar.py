...
class VAR(nn.Module):
    def __init__(
        ...
        # 3. absolute position embedding TODO 更改为RelativePositionBias
        pos_1LC = []
        for i, pn in enumerate(self.patch_nums):
...
class MAR(L.LightningModule):
    def __init__(self, vae_local: VQVAE,
                 num_classes=1000, depth=16, embed_dim=1024, num_heads=16, mlp_ratio=4., drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0.,
                 norm_eps=1e-6, shared_aln=False, cond_drop_rate=0.1,
                 attn_l2_norm=False,
                 patch_nums=(1, 2, 3, 4, 5, 6, 8, 10, 13, 16),  # 10 steps by default
                 flash_if_available=True, fused_if_available=True, lr=0.0001):
        super().__init__()
        self.vae_local = vae_local
        self.quantize_local = vae_local.quantize
        self.var = VAR(vae_local=vae_local,
                       num_classes=num_classes, depth=depth, embed_dim=embed_dim, num_heads=num_heads,
                       mlp_ratio=mlp_ratio,
                       ...
                       flash_if_available=flash_if_available, fused_if_available=fused_if_available)
        self.loss = nn.CrossEntropyLoss(label_smoothing=0.0, reduction='none')
    ...
    def forward(self, label_B, x_BLCv_wo_first_l):
        return self.var(label_B, x_BLCv_wo_first_l)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = self.loss(logits_BLV.view(-1, V), gt_BL.view(-1)).view(B, -1)
        lw = self.loss_weight.to(loss.device)
    ...
    def validation_step(self,  batch, batch_idx):
        ...
        self.log_dict({'val_loss': L_mean, 'val_acc': acc_mean, 'L_tail': L_tail, 'acc_tail': acc_tail}, prog_bar=True)

    def configure_optimizers(self):
        return torch.optim.AdamW(self.parameters(), lr=self.lr, betas=(0.9, 0.95), fused=True)
